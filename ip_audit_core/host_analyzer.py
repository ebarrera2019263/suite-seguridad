...
from . import connectivity, ddos_exposure, discovery, os_detector, smb_analyzer, sqli_analyzer, ssh_analyzer, ssl_analyzer
...
def analyze_host(
    ...
    if check_sqli:
        _run_check("SQL Injection", result, _check_sqli, result, banner_timeout_s)
    if check_ddos_exposure:
        _run_check("Exposicion a DDoS (amplificacion)", result, _check_ddos_exposure, result, tls_timeout_s)
    if check_connectivity:
        _run_check("Conectividad (blackbox)", result, _check_connectivity, result)
...
def _check_sqli(result: HostResult, timeout_s: float) -> None:
    """Sondeo error-based NO destructivo de SQL injection sobre los puertos
    web abiertos. No autentica, no extrae datos, no altera la base: solo envia
    payloads que a lo sumo provocan un error de sintaxis y observa si el
    servidor filtra un mensaje de error SQL. Ver sqli_analyzer para limites."""
    web_ports = [p for p in result.open_ports if p in sqli_analyzer.WEB_PORTS]
    for port in web_ports:
        findings = sqli_analyzer.analyze_sqli(result.ip, port, timeout_s=max(timeout_s, 4.0))
        for err in findings.errors:
            result.errors.append(f"SQLi:{port}: {err}")
        if not findings.reachable:
            continue
        if findings.vulnerable:
            result.add_vuln(
                Vulnerability(
                    title=f"Posible SQL injection (error-based) en aplicacion web (puerto {port})",
                    category="Inyeccion / validacion de entrada (OWASP A03)",
                    severity=Severity.HIGH,
                    description=(
                        f"El servidor devolvio un mensaje de error de base de datos "
                        f"({findings.dbms}) al enviar un payload de prueba en el parametro "
                        f"'{findings.param}'. Esto indica que la entrada del usuario llega sin "
                        "sanitizar a una consulta SQL, condicion tipica de una vulnerabilidad "
                        "de inyeccion SQL. No se extrajeron datos: la prueba solo provoca el error."
                    ),
                    recommendation=(
                        "Usar consultas parametrizadas / sentencias preparadas (nunca concatenar "
                        "entrada del usuario en el SQL), validar y escapar entradas, aplicar el "
                        "principio de minimo privilegio en la cuenta de BD, y ocultar los mensajes "
                        "de error detallados al cliente. Confirmar con una prueba manual dirigida "
                        "(p.ej. sqlmap) antes de remediar."
                    ),
                    port=port,
                    evidence=f"payload '{findings.payload}' en '{findings.param}' -> {findings.evidence}",
                )
            )
        else:
            # Deja constancia de que la prueba se corrio (cobertura visible en
            # el reporte), aunque no haya hallazgo.
            result.add_vuln(
                Vulnerability(
                    title=f"Prueba de SQL injection realizada sin hallazgos error-based (puerto {port})",
                    category="Inyeccion / validacion de entrada (OWASP A03)",
                    severity=Severity.INFO,
                    description=(
                        "Se sondeo la aplicacion web con payloads de prueba en parametros comunes "
                        "y no se detecto SQL injection basada en errores. Es una prueba liviana: no "
                        "descarta SQLi ciega (booleana/temporal) ni parametros/formularios no "
                        "cubiertos."
                    ),
                    recommendation=(
                        "Para cobertura completa, complementar con un DAST dedicado (OWASP ZAP) o "
                        "sqlmap sobre los endpoints con parametros reales de la aplicacion."
                    ),
                    port=port,
                    evidence=(f"Server: {findings.http_server}" if findings.http_server else None),
                )
            )


def _check_ddos_exposure(result: HostResult, timeout_s: float) -> None:
    """Detecta si el host expone servicios UDP de amplificacion (DNS/NTP/SSDP/
    SNMP/memcached/CharGen) que podrian abusarse para reflejar y multiplicar un
    DDoS contra terceros. Envia UN paquete benigno por servicio; NO inunda ni
    ataca. Ver ddos_exposure para el detalle."""
    findings = ddos_exposure.analyze_ddos_exposure(result.ip, timeout_s=min(timeout_s, 2.5))
    for err in findings.errors:
        result.errors.append(f"DDoS-exposicion: {err}")
    for exp in findings.exposures:
        severity = Severity.HIGH if exp.factor >= 50 else Severity.MEDIUM
        result.add_vuln(
            Vulnerability(
                title=f"Servicio amplificador de DDoS expuesto: {exp.service} (puerto {exp.port}/udp)",
                category="Exposicion a denegacion de servicio (DDoS por amplificacion)",
                severity=severity,
                description=(
                    f"El servicio {exp.service} respondio a un unico paquete de prueba con una "
                    f"respuesta {exp.factor}x mas grande que la peticion "
                    f"({exp.request_bytes}->{exp.response_bytes} bytes). Un atacante puede "
                    f"falsificar la IP de una victima y usar este host como reflector para "
                    f"multiplicar el volumen de un ataque DDoS contra ella (factor tipico "
                    f"documentado: {exp.typical_factor}). No se realizo ningun ataque: solo se "
                    f"midio la amplificacion con un paquete."
                ),
                recommendation=(
                    "Restringir el servicio a la red interna (firewall/ACL), deshabilitarlo si no "
                    "es necesario, y desactivar funciones abusables (recursion DNS abierta, "
                    "'monlist' en NTP, respuesta a 'stats' en memcached expuesto). Aplicar BCP38 "
                    "(anti-spoofing) en el borde de red."
                ),
                port=exp.port,
                evidence=f"amplificacion medida: {exp.factor}x ({exp.request_bytes}->{exp.response_bytes} bytes)",
            )
        )


def _check_connectivity(result: HostResult) -> None:
    """Prueba de caja negra que verifica que el host es alcanzable y que los
    paquetes se envian y regresan (ICMP + handshake TCP + sondeo UDP), con una
    cantidad ACOTADA de paquetes. No es un ataque; es un diagnostico de red.
    Deja un hallazgo informativo con las metricas (enviados/recibidos/perdida/
    latencia) para dejar constancia del alcance real al objetivo."""
    tcp_ports = result.open_ports or [443, 80, 445, 22]
    probe = connectivity.connectivity_test(result.ip, tcp_ports=tcp_ports, count=4, timeout_s=1.5)
    estado = "alcanzable" if probe.reachable else "SIN respuesta a los paquetes de prueba"
    result.add_vuln(
        Vulnerability(
            title=f"Prueba de conectividad blackbox: host {estado}",
            category="Conectividad / envio de paquetes (prueba blackbox)",
            severity=Severity.INFO,
            description=(
                "Verificacion de caja negra del alcance al objetivo enviando una cantidad "
                "acotada de paquetes (ICMP, handshake TCP y sondeo UDP) y midiendo cuantos "
                f"regresaron. Resultado: {probe.summary()}."
            ),
            recommendation=(
                "Si el host responde a paquetes que no deberia aceptar desde esta red "
                "(p.ej. ICMP o puertos de gestion), considerar filtrarlos en el firewall. "
                "Si NO responde y deberia, revisar reglas de firewall/routing."
            ),
            evidence=probe.summary(),
        )
    )
    for note in probe.notes:
        result.errors.append(f"Conectividad: {note}")
